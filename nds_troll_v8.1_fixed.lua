--[[
    ╔═══════════════════════════════════════════════════════════════════════════╗
    ║                     NDS TROLL HUB v8.1 - GUI FIXED                        ║
    ║                   Natural Disaster Survival                               ║
    ║              Compatível com Executores Mobile e PC                        ║
    ║                                                                           ║
    ║  CHANGELOG v8.1:                                                          ║
    ║  - CORRIGIDO: Botões não respondiam ao clique                             ║
    ║  - CORRIGIDO: Botão minimizar não funcionava                              ║
    ║  - CORRIGIDO: Textos e botões não responsivos                             ║
    ║  - MELHORADO: GUI completamente reescrita para compatibilidade            ║
    ║  - MELHORADO: Eventos de input unificados (Mouse + Touch)                 ║
    ╚═══════════════════════════════════════════════════════════════════════════╝
--]]
...
if genv.NDS_TROLL_HUB_V8_LOADED then
    warn("[NDS v8.1] Script já está carregado!")
    return
end
...
local function ToggleOrbit()
    ...
    if State.Orbit then
        ...
        for _, part in pairs(GetAvailableParts()) do
            ...
            if attach and align then controlledParts[part] = true end
        end
            ...
            if State.Orbit and obj:IsA("BasePart") then
                task.defer(function()
                    if not obj.Anchored and not IsMapPart(obj) then
                        ...
                        if attach and align then controlledParts[obj] = true end
                    end
                end)
            end
        end)
    ...
    if State.Cage then
        ...
        for ring = 1, rings do
            ...
            for i = 1, partsPerRing do
                ...
                table.insert(cageAttachments, {attach = attachment, angle = angle, height = ringHeight})
        ...
        for _, cageData in ipairs(cageAttachments) do
            if partIndex <= #parts then
                SetupPartControl(parts[partIndex], cageData.attach, "normal")
...
local function ToggleHatFling()
    ...
    if State.HatFling then
        ...
        ClearConnections("HatFling")
        for _, handle in pairs(GetMyAccessories()) do
    ...
    if State.ServerMagnet then
        ...
        Connections.ServerMagnetNew = Workspace.DescendantAdded:Connect(function(obj)
            if State.ServerMagnet and obj:IsA("BasePart") then
                task.defer(function()
                    if not obj.Anchored and not IsMapPart(obj) then
                        SetupPartControl(obj, serverAttachment, "normal")
                    end
                end)
            end
        end)
...
local function ToggleTelekinesis()
    ...
    if State.Telekinesis then
        ...
        Connections.TeleClick = UserInputService.InputBegan:Connect(function(input, gameProcessed)
...
-- ═══════════════════════════════════════════════════════════════════════════
-- INTERFACE GRÁFICA (GUI) - CORRIGIDA v8.1
-- ═══════════════════════════════════════════════════════════════════════════
...
local function CreateGUI()
    ...
    local buttonWidth = platform.isMobile and 140 or 120
    local buttonHeight = platform.isMobile and 50 or 40
    local fontSize = platform.isMobile and 14 or 12
    local padding = platform.isMobile and 8 or 6
    local mainWidth = platform.isMobile and 320 or 280
    local mainHeight = platform.isMobile and 480 or 420
    ...
    platformLabel.Size = UDim2.new(0, 60, 0, 22)
    platformLabel.Position = UDim2.new(1, -110, 0.5, -11)
    ...
    minimizeBtn.Position = UDim2.new(1, -45, 0.5, -18)
    ...
    dropdownBtn.Position = UDim2.new(1, -40, 0.5, -18)
    ...
    -- ═══════════════════════════════════════════════════════════════════════
    -- SISTEMA DE DRAG (ARRASTAR) - CORRIGIDO
    -- ═══════════════════════════════════════════════════════════════════════
    ...
    local function ToggleMinimize()
        ...
        if isMinimized then
            mainFrame.Size = UDim2.new(0, mainWidth, 0, 50)
    ...
    local function CreatePlayerButton(player)
        ...
        local function SelectPlayer()
            ...
            playerList.Visible = false
            Notify("NDS v8.1", "Player selecionado: " .. player.Name, 2)
        end
        
        btn.MouseButton1Click:Connect(SelectPlayer)
        btn.Activated:Connect(SelectPlayer)
    ...
    dropdownBtn.Activated:Connect(ToggleDropdown)
    
    -- ═══════════════════════════════════════════════════════════════════════
    -- CRIAÇÃO DE BOTÕES DE FUNÇÃO - CORRIGIDO
    -- ═══════════════════════════════════════════════════════════════════════
    ...
    disableAllBtn.AutoButtonColor = true
    ...
    local function OnDisableAll()
        ...
        for name, btn in pairs(buttonRefs) do
            btn.BackgroundColor3 = Color3.fromRGB(50, 50, 70)
        end
        
        Notify("NDS v8.1", "Todas as funções desativadas!", 2)
    end
    
    disableAllBtn.MouseButton1Click:Connect(OnDisableAll)
    disableAllBtn.Activated:Connect(OnDisableAll)
    ...
    Players.PlayerRemoving:Connect(function(player)
        if State.SelectedPlayer == player then
            ...
            DisableAllFunctions()
            SetupNetworkControl()
            Notify("NDS v8.1", "Player selecionado saiu do jogo!", 3)
...
local function Initialize()
    ...
    Notify("NDS TROLL HUB v8.1", "Carregado com sucesso!\nPlataforma: " .. platformText .. executorText, 5)
    
    -- Avisos de compatibilidade
    if not platform.hasSetHiddenProperty and not platform.hasSetSimRadius then
        task.wait(1)
        Notify("Aviso", "SimulationRadius limitado. Algumas funções podem ter alcance reduzido.", 5)
    end
    
    -- Log de debug
    print("═══════════════════════════════════════════════════")
    print("NDS TROLL HUB v8.1 - GUI FIXED")
    print("═══════════════════════════════════════════════════")
    ...
    print("═══════════════════════════════════════════════════")
    print("")
    print("CHANGELOG v8.1:")
    print("- CORRIGIDO: Botões não respondiam ao clique")
    print("- CORRIGIDO: Botão minimizar não funcionava")
    print("- CORRIGIDO: GUI não era responsiva")
    print("- MELHORADO: Eventos unificados (Mouse + Touch + Activated)")
    print("- MELHORADO: Sistema de drag reescrito")
    print("═══════════════════════════════════════════════════")
end
